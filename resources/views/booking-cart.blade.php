        ...
        <div class="space-y-6">
            @foreach($cartItems as $item)
                <div class="flex items-center bg-white shadow rounded-lg p-4 gap-6 hover:shadow-lg transition">
                    <img src="{{ $item->service->image_url ?? 'https://via.placeholder.com/100x80?text=No+Image' }}"
                         alt="Service Image"
                         class="w-24 h-20 object-cover rounded-md border border-gray-200">

                    <div class="flex-grow">
                        <h3 class="text-lg font-semibold text-gray-800">{{ $item->service->name }}</h3>
                        <p class="text-gray-700 mt-1 font-semibold">RM{{ number_format($item->total_price, 2) }}</p>
                    </div>
