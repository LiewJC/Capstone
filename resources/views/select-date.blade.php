...
<div class="flex-grow max-w-4xl mx-auto p-6">
    ...
    @if(session('error'))
        <div class="mb-6 text-center text-red-600 font-semibold">{{ session('error') }}</div>
    @endif
    ...
    <div class="text-center mb-8">
        <strong>Total Duration:</strong> {{ $totalDuration }} mins (Rounded: {{ $roundedDuration }} mins)
    </div>
    ...
    <div class="flex flex-col md:flex-row md:space-x-10 justify-center items-start mb-10">
        ...
        <!-- Booking form -->
        <form id="bookingForm" method="GET" action="{{ route('payment.page') }}" class="w-full max-w-md bg-white rounded-lg border border-gray-300 p-6 shadow-sm">
