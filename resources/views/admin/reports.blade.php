...
@section('content')
    ...
    <form method="GET" action="{{ route('admin.report') }}" class="mb-8 flex flex-wrap items-center space-x-4 space-y-2">
        <label class="flex flex-col text-sm font-medium text-gray-700">
            From:
            <input type="date" name="from" value="{{ request('from') }}" class="border border-gray-300 rounded px-3 py-1" />
        </label>
        <label class="flex flex-col text-sm font-medium text-gray-700">
            To:
            <input type="date" name="to" value="{{ request('to') }}" class="border border-gray-300 rounded px-3 py-1" />
        </label>
    ...
    <div class="mb-10 text-center space-y-2">
        <h3 class="text-xl font-semibold text-gray-700">Total Sales: <span class="text-primary-700">RM
                {{ number_format($totalSales, 2) }}</span></h3>
        <h3 class="text-xl font-semibold text-gray-700">Total Bookings: <span
                class="text-primary-700">{{ $totalBookings }}</span></h3>
    </div>
    ...
    <script>
        const dailyLabels = {!! json_encode($dailySalesLabels) !!};
