...
<div class="flex flex-col items-center justify-center min-h-full py-12 px-4">
    ...
    <form action="{{ route('admin.stores.store') }}" method="POST" class="max-w-4xl w-full bg-white rounded shadow p-6 space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-3 gap-2">
                @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                    <label class="inline-flex items-center space-x-2">
                        <input type="checkbox" name="days[]" value="{{ $day }}" class="form-checkbox">
                        <span>{{ $day }}</span>
                    </label>
                @endforeach
            </div>
