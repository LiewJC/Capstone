    ...
    <main class="max-w-7xl mx-auto px-6 py-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            @foreach ($services as $service)
                ...
                @if($feedbacksWithComments->isNotEmpty())
                    <div class="bg-white rounded-lg p-8 shadow-md hover:shadow-xl transition">
                        <h3 class="text-2xl font-semibold mb-2 text-blue-600">{{ $service->name }}</h3>
                        <p class="text-gray-700 mb-1">{{ $service->description }}</p>
                        <p class="text-green-600 font-bold mb-4">Price: RM{{ number_format($service->price, 2) }}</p>

                        @if($avgRating)
                            <p class="text-yellow-500 font-semibold mb-4">⭐ Average Rating: {{ number_format($avgRating, 1) }}/5</p>
                        @endif
                        ...
                        @foreach($latestComments as $comment)
                            <div class="bg-gray-100 rounded-md p-4 mb-4 border-l-4 border-blue-600 italic">
                                <p class="font-semibold">{{ $comment->user->user_name ?? 'Anonymous' }}</p>
                                <p class="text-yellow-400">⭐ {{ $comment->rating }}/5</p>
